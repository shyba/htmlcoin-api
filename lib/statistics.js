...
function StatisticsController(options) {
	...
	this.node.services.htmlcoind.on('tip', this._rapidProtectedUpdateTip.bind(this));
...
StatisticsController.prototype.totalSupply = function(req, res) {
    var supply, newHeightSinceGenesis, newCoinSinceGenesis;
    var height = this.node.services.htmlcoind.height;
    newHeightSinceGenesis = height - 798;
    newCoinSinceGenesis = newHeightSinceGenesis * 1250;
    supply = newCoinSinceGenesis + 79800000000;
...
StatisticsController.prototype.difficulty = function(req, res) {
    ...
    while(self.statisticByDays.get(formattedDate) && days > iterator) {
        ...
        var cachedDay = self.statisticByDays.get(formattedDate),
            ...
            sumPoW = cachedDay.difficultyPoW && cachedDay.difficultyPoW.sum && cachedDay.difficultyPoW.count ? cachedDay.difficultyPoW.sum / cachedDay.difficultyPoW.count : 0;
...
StatisticsController.prototype.total = function(req, res) {
    ...
    while(next && height > 0) {
        ...
        if (currentElement) {
            ...
            if (currentElement.flags === bitcore.Block.PROOF_OF_STAKE && difficulty) {
                ...
                /*PoW*/
                sumDifficultyPoW += difficulty;
                countDifficultyPoW++;
            }

            if (subsidy) {//TODO: separate from PoW for HTML?
	...
	var result = {
        ...
        difficulty_pow: sumDifficultyPoW && countDifficultyPoW ? sumDifficultyPoW / countDifficultyPoW : 0,
		...
		return async.waterfall([function (callback) {
            ...
            if (block.difficulty) {
                if(block.flags === bitcore.Block.PROOF_OF_STAKE) {
                    ...
                    cachedStatisticDay.difficultyPoW.sum += block.difficulty;
                    cachedStatisticDay.difficultyPoW.count += 1;
                }
